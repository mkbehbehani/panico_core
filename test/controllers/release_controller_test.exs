defmodule PanicoCore.ReleaseControllerTest do
  use PanicoCore.ConnCase
  ...
  test "creates resource and redirects when data is valid", %{conn: conn} do
    conn = post conn, release_path(conn, :create), release: @valid_attrs
    assert redirected_to(conn) == release_path(conn, :index)
    assert Repo.get_by(Release, @valid_attrs)
  end
  ...
  test "renders page not found when id is nonexistent", %{conn: conn} do
    assert_error_sent 404, fn ->
      get conn, release_path(conn, :show, -1)
    end
  end
  ...
  test "updates chosen resource and redirects when data is valid", %{conn: conn} do
    release = Repo.insert! %Release{}
    conn = put conn, release_path(conn, :update, release), release: @valid_attrs
